...
const Czerwiec2025Podstawa = ({
  ...
  const [showAllProblems, setShowAllProblems] = useState(() => {
    try {
      const saved = localStorage.getItem('czerwiec2025podstawa-show-all-problems');
  ...
  useEffect(() => {
    const savedSuggestions = localStorage.getItem('czerwiec2025podstawa-suggested-problems');
    if (savedSuggestions) {
      try {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('czerwiec2025podstawa-show-all-problems', JSON.stringify(showAllProblems));
    } catch (e) {
...
export default Czerwiec2025Podstawa;