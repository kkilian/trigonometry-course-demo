...
const Maj2025Podstawa = ({
  ...
  const [showAllProblems, setShowAllProblems] = useState(() => {
    try {
      const saved = localStorage.getItem('maj2025podstawa-show-all-problems');
  ...
  useEffect(() => {
    const savedSuggestions = localStorage.getItem('maj2025podstawa-suggested-problems');
    if (savedSuggestions) {
      try {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('maj2025podstawa-show-all-problems', JSON.stringify(showAllProblems));
    } catch (e) {
...
export default Maj2025Podstawa;