...
const KombinatorykRozszerzenieStartHere = ({
  problems,
  onSelectProblem,
  ...
  useEffect(() => {
    const savedSuggestions = localStorage.getItem('kombinatoryka-rozszerzenie-suggested-problems');
    if (savedSuggestions) {
      try {
    ...
    <div className="min-h-screen bg-stone-100">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-8 md:py-16">
        <div className="space-y-8">
          ...
          {/* Twoje zadania - pokazuj tylko gdy są jakieś ukończone i nie pokazujemy wszystkich */}
          {completedProblems.size > 0 && !showAllProblems && (
            <div className="mt-12 px-4 md:px-8">
              <div className="border-t border-stone-200 pt-8">
                <h3 className="text-lg font-semibold text-stone-800 mb-2">Twoje zadania</h3>
                <p className="text-stone-600 text-sm mb-6">Ostatnio rozwiązane zadania</p>

                <div className="grid md:grid-cols-2 gap-3">
                  {problems
                    .filter(p => completedProblems.has(p.id))
                    .slice(-5) // Ostatnie 5 rozwiązanych
                    ...
                    .map((problem, index) => {
                      ...
                      return (
                        <button
                          key={problem.id}
                          onClick={() => handleStartProblem(problem)}
                          className="text-left p-4 bg-green-50 border border-green-200 hover:border-green-300 hover:bg-green-100 rounded-lg transition-all group"
                        >
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="mt-1 text-xs text-green-600">
                                {problem.steps?.length || 0} kroków • Rozwiązane
                              </div>
                            </div>
                            <div className="flex-shrink-0">
                              <div className="w-6 h-6 rounded-full bg-green-100 group-hover:bg-green-200 flex items-center justify-center transition-all">
                                <svg className="w-3 h-3 text-green-600 group-hover:text-green-700 transition-colors" fill="none" viewBox="0 0 20 20">
                                  <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 5l6 5-6 5" />
                                </svg>
                              </div>
                            </div>
                ...
                {completedProblems.size > 5 && (
                  <div className="mt-4 text-center">
                    <p className="text-xs text-stone-500">
                      Pokazano 5 z {completedProblems.size} rozwiązanych zadań
                    </p>
                  </div>
                )}
              </div>
...
export default KombinatorykRozszerzenieStartHere;