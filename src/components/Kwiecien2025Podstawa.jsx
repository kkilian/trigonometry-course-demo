...
const Kwiecien2025Podstawa = ({
  ...
  const [showAllProblems, setShowAllProblems] = useState(() => {
    try {
      const saved = localStorage.getItem('kwiecien2025podstawa-show-all-problems');
  ...
  useEffect(() => {
    const savedSuggestions = localStorage.getItem('kwiecien2025podstawa-suggested-problems');
    if (savedSuggestions) {
      try {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('kwiecien2025podstawa-show-all-problems', JSON.stringify(showAllProblems));
    } catch (e) {
...
export default Kwiecien2025Podstawa;