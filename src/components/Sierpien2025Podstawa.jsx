...
const Sierpien2025Podstawa = ({
  ...
  const [showAllProblems, setShowAllProblems] = useState(() => {
    try {
      const saved = localStorage.getItem('sierpien2025podstawa-show-all-problems');
  ...
  useEffect(() => {
    const savedSuggestions = localStorage.getItem('sierpien2025podstawa-suggested-problems');
    if (savedSuggestions) {
      try {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('sierpien2025podstawa-show-all-problems', JSON.stringify(showAllProblems));
    } catch (e) {
